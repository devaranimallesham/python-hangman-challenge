...
function Index() {
  ...
  const [clue, setClue] = useState((THEMES[0] ?? FALLBACK_THEME).words[0]?.hint ?? FALLBACK_WORD.hint);
...
interface PlayViewProps {
  mode: Mode; themeName: string; level: (typeof LEVELS)[number]; word: string; clue: string; revealedWord: string[]; guessed: Set<string>; wrongGuesses: number; maxChances: number; hintUsed: boolean; status: GameStatus; score: number; playerName: string;
  onModeChange: (mode: Mode) => void; onGuess: (letter: string) => void; onHint: () => void; onRestart: () => void;
}

function PlayView({ mode, themeName, level, word, clue, revealedWord, guessed, wrongGuesses, maxChances, hintUsed, status, score, playerName, onModeChange, onGuess, onHint, onRestart }: PlayViewProps) {
